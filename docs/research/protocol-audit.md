...
| Pad address translation | [ep133-ppak/PROTOCOL.md](https://github.com/ZacharySBrown/ep133-ppak/blob/93f0f6b1f6110701e6378e2702f39fd7e9a9d687/PROTOCOL.md) maps physical label 1 to SysEx position 7 and TAR p04. Source. Read-node numbering resolved on our hardware: see [backup verification](backup-verification.md). | **Read and write use different pad numbers.** Metadata node ids follow TAR `pNN`, so project 1 / group A / label 1 is node **3204**, not 3207. `assign_pad` still takes SysEx `pad_num` 7. Verify the written pad by reading the TAR-numbered node back. |
...
- Metadata read nodes are numbered by the project TAR `pNN`, while `assign_pad` takes the SysEx `pad_num`. Do not reuse one number for the other; see [backup verification](backup-verification.md).
